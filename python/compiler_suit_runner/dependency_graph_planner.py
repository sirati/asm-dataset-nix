...
def _load_source_terminal_predicate():
    ...
    Returns a callable taking a single ``role: str`` argument. The
    template_graph package is the natural source of truth for the
    arch-indep terminal patterns (source tarballs, fetchurl, builder
    scripts, patches, setup-hooks). We import lazily so the planner
    module stays loadable in environments where ``template_graph`` is
    absent (unit tests that exercise the adapter without the streaming
    package); the fallback is a no-skip predicate so behaviour stays
    deterministic — every arch-indep ident becomes a task — and the
    operator can spot the gap in the spawn log.
    """
    try:
        ...
        return lambda _role: False, lambda name: name
