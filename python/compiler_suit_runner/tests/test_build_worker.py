...
# ---------------------------------------------------------------------------
# PH-C: placeholder resolution
# ---------------------------------------------------------------------------


def _write_sidecar(
    path: pathlib.Path, *, binary: str, compiler: str, arch: str,
    variant_payloads: list[dict],
) -> pathlib.Path:
    """Write a per-cell sidecar manifest in the shape that
    :func:`workers.dependency_graph_worker.output.write_per_cell_manifests`
    produces. ``variant_payloads`` is the inner ``payload`` for each
    slot; the test fills in the surrounding scaffolding so the worker
    sees the same on-disk shape it would on the cluster."""
    body = {
        "binary": binary,
        "compiler": compiler,
        "arch": arch,
        "variants": [
            {
                ...
                "kind": "build_variant",
                "task_id": (
                    f"build_variant__x86_64-linux__{binary}__"
                    f"{compiler}__{arch}__slot{idx}"
                ),
                "name": p.get("label") or f"slot{idx}",
                "payload": p,
                "depends_on": [],
            }
            for idx, p in enumerate(variant_payloads)
        ],
    }
...
def test_resolve_placeholder_payload_returns_resolved_payload(tmp_path):
    sidecar = _write_sidecar(
        tmp_path / "_manifests" / "hello__gcc15__x86_64.json",
        binary="hello",
        compiler="gcc15",
        arch="x86_64",
        variant_payloads=[
            {
                "pkg": "hello",
                "arch": "x86_64",
                "label": "hello__gcc15__O2",
                "drv": "/nix/store/abc-hello.drv",
                "variant_dir": "gcc15__O2",
                "metadata_name": "hello__gcc15__O2",
                "compiler_id": "gcc15",
                "tier": 0,
                "attr": "dataset.x86_64-linux.hello.x86_64.gcc15__O2",
            },
        ],
    )
    placeholder = {
    ...
    resolved = bw._resolve_placeholder_payload(placeholder)
    assert resolved is not None
    assert resolved["drv"] == "/nix/store/abc-hello.drv"
    assert resolved["variant_dir"] == "gcc15__O2"
    assert resolved["label"] == "hello__gcc15__O2"


def test_resolve_placeholder_payload_out_of_range_returns_none(tmp_path):
    sidecar = _write_sidecar(
        tmp_path / "_manifests" / "hello__gcc15__x86_64.json",
        binary="hello",
        compiler="gcc15",
        arch="x86_64",
        variant_payloads=[
            {"drv": "/nix/store/abc-hello.drv", "label": "v0"},
        ],
    )
    placeholder = {
        "placeholder": True,
    ...
    assert bw._resolve_placeholder_payload(placeholder) is None


def test_resolve_placeholder_payload_missing_sidecar_returns_none(tmp_path):
    placeholder = {
        "placeholder": True,
        "slot_idx": 0,
...
def test_resolve_placeholder_payload_unreadable_raises(tmp_path):
    sidecar = tmp_path / "_manifests" / "hello__gcc15__x86_64.json"
    sidecar.parent.mkdir(parents=True, exist_ok=True)
    sidecar.write_text("not-json-here", encoding="utf-8")
    placeholder = {
        "placeholder": True,
        "slot_idx": 0,
        "manifest_path": str(sidecar),
    ...
    with pytest.raises(ValueError, match="unreadable"):
        bw._resolve_placeholder_payload(placeholder)


def test_resolve_placeholder_payload_wrong_kind_raises(tmp_path):
    sidecar = tmp_path / "_manifests" / "hello____common____x86_64.json"
    sidecar.parent.mkdir(parents=True, exist_ok=True)
    body = {
        "binary": "hello",
        "compiler": "__common__",
    ...
    with pytest.raises(ValueError, match="expected 'build_variant'"):
        bw._resolve_placeholder_payload(placeholder)


def test_resolve_placeholder_payload_missing_manifest_path_raises():
    with pytest.raises(ValueError, match="manifest_path"):
        bw._resolve_placeholder_payload(
            {"placeholder": True, "slot_idx": 0}
        )


def test_resolve_placeholder_payload_missing_slot_idx_raises(tmp_path):
    sidecar = _write_sidecar(
        tmp_path / "_manifests" / "hello__gcc15__x86_64.json",
        binary="hello", compiler="gcc15", arch="x86_64",
        variant_payloads=[{"drv": "x.drv"}],
    )
...
def test_build_worker_placeholder_out_of_range_yields_no_op(tmp_path):
    sidecar = _write_sidecar(
        tmp_path / "_manifests" / "hello__gcc15__x86_64.json",
        binary="hello", compiler="gcc15", arch="x86_64",
        variant_payloads=[],  # zero variants → any slot is out of range
    )
    placeholder_payload = {
        "placeholder": True,
        "slot_idx": 0,
    ...
    manifest_path = _write_manifest(
        tmp_path / "ph.json",
        item_class=ITEM_CLASS_BUILD_VARIANT,
        name="build_variant__hello__gcc15__x86_64__slot0",
        payload=placeholder_payload,
    )
    env = BuildWorkerEnv(
        flake_ref=".",
        dataset_output_dir=tmp_path / "dataset",
        run_subprocess=RecordingRunner(),  # would never be invoked
        clock=FakeClock(),
    )
    result = build_worker(manifest_path, env)
    assert result.success is True
    assert result.no_op is True
    # Confirm nix build was NOT invoked for an out-of-range slot.
    assert env.run_subprocess.calls == []


def test_build_worker_placeholder_in_range_proceeds_to_build(tmp_path):
    sidecar = _write_sidecar(
        tmp_path / "_manifests" / "hello__gcc15__x86_64.json",
        binary="hello", compiler="gcc15", arch="x86_64",
        variant_payloads=[
    ...
    # nix build "succeeds" with a fake out-path on stdout; the worker
    # then walks the copy_elf_folder path. Use a tempfile as the fake
    # /nix/store/.../elf so copy_elf_folder finds something.
    fake_store = tmp_path / "store-out"
    fake_store.mkdir()
    elf = fake_store / "hello"
    elf.write_bytes(b"\x7fELF")  # ELF magic so _find_elf_dir accepts
    runner = RecordingRunner(stdout=(str(fake_store) + "\n").encode())
    env = BuildWorkerEnv(
        flake_ref=".",
        dataset_output_dir=tmp_path / "dataset",
        run_subprocess=runner,
    ...
    assert runner.calls, (
        "expected nix build to be invoked when placeholder resolves "
        "to an in-range slot"
    )
    # Verify argv carried the resolved drv path (not the placeholder).
    argv0 = runner.calls[0]
    assert any(
        arg.endswith("/nix/store/abc-hello.drv^*") for arg in argv0
    ), argv0
