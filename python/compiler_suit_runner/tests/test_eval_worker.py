...
def _eval_worker_env(monkeypatch, tmp_path):
    ...
    * neutralise the toolchain IMPORT (``_import_toolchain_archive``) to
    ...
    _bw._common_archive_imported = False
    _bw._imported_toolchain_out_paths.clear()
    yield
    _bw._toolchain_imported = False
    _bw._common_archive_imported = False
    _bw._imported_toolchain_out_paths.clear()
...
# ---------------------------------------------------------------------------
# _import_toolchain_archive (drv archive import on dedup path)
# ---------------------------------------------------------------------------


def test_eval_worker_drv_archive_imported_on_dedup_path(
    tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The shared ``toolchains.drv.archive`` is imported when toolchain_dedup
    is ON.  Realized-output archives (common + per-toolchain deltas) are the
    build_worker's concern; the eval worker only needs the drv archive to
    run ``nix-instantiate`` on the matrix aggregate.
    """
    _install_wrapper_stub(monkeypatch)
    order: list[str] = []
    ...
    # Override the autouse no-op with a recording stub.
