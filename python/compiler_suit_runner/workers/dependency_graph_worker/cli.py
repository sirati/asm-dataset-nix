...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--toolchain-drv",
        dest="toolchain_aggregate_drv",
        type=str,
        required=True,
        help=(
            "Toolchain aggregate .drv path (e.g."
            " /nix/store/<hash>-toolchains.drv). Single value, required."
            " Replaces the previous repeated per-toolchain flag: the"
            " watcher now supplies a pre-built aggregate that wraps"
            " every active toolchain in one drv."
        ),
    )
    parser.add_argument(
        "--matrix-drv",
