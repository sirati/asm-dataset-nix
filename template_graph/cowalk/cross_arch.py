...
The A/B/C/D classifier (:func:`_classify_cross_arch_sharing`, lifted
from ``template_graph.streaming._helpers`` and re-exported there for
back-compat) runs unchanged on the per-arch variant-0 idents at each
position. We project the result onto ``MetaTemplate.cross_arch_classification``:
