...
`--important-stdio-only` is a **dynamic_runner CLI FLAG** — there is NO `DYNRUNNER_IMPORTANT_STDIO_ONLY` operator env var (the flag sets a framework-internal toggle for you; do not invent the env var). `compiler_suit_runner`'s argparse accepts it and passes it through: it survives `_strip_csr_argv_for_framework` because it is deliberately NOT in `_CSR_BOOL_FLAGS`, and the framework drops it from the secondary argv (`_forwarded_argv.SUBMITTER_LOCAL_FLAGS`) so grid workers keep FULL logs.

**FORBIDDEN (each looked right at first and was not):**
...
- On relocated runs the dispatcher's terminal `run complete: N succeeded` line and the exit accounting can read ALL-ZEROS even on full success. Do NOT trust them. (Sometimes the accounting is intact — never rely on either way.)
...
- On `dynamic_runner` ≥ `835f269c` the relocated primary's full per-role logs land durably at `<slurm-root>/log/run_<ts>/<secondary_id>/{primary,secondary}.log` (the framework injects `DYNRUNNER_FULL_LOG_DIR`); read those for post-promotion detail. Older pins discard them — fall back to the per-secondary `slurm_<jobid>.{out,err}`.

(Legacy note: do NOT revert to `tail -f /tmp/csr-dispatch-*.log | grep …` against a backgrounded dispatch — that's the old shape this section replaced.)
